...
class Rocket:
    def __init__(self, screen):
        ...
        self.explosion_image = pygame.transform.scale(self.explosion_image, (100, 100))
...
class Bomb:
    def __init__(self, screen, ufo):
        self.screen = screen
        self.image = pygame.image.load("bomb.png")
        self.rect = self.image.get_rect()
        self.rect.centerx = ufo.rect.centerx
        self.rect.top = ufo.rect.bottom
...
class UFO:
    def __init__(self, screen):
        ...
        self.bomb_timer = 0  # Timer for bomb dropping
        self.bomb_interval = random.randint(1,50)  # Random interval for bomb dropping
...
# Create a list to store bombs
bombs = []

# Game loop.
running = True
rocket_hit = False  # Initialize rocket_hit variable
while running:
    ...
    for bomb in bombs:
        bomb.move()
        bomb.draw()
        if bomb.rect.colliderect(rocket.rect):
            # Handle rocket hit by a bomb
            rocket_hit = True
            break
        if bomb.rect.top > screen.get_height():
            bombs.remove(bomb)

    # Check if rocket was hit by a bomb
    if rocket_hit:
        rocket.explode()
